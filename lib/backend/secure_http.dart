...
class SecureHttp {
  ...
  static Future<PublicHandshakeResult> pseudoTlsHandshake(
    ...
    try {
      ...
      final BigInt n; // Modulus (part of server's public key).
      final BigInt e; // Public exponent (part of server's public key).
      ...
      final encryptedPreMasterSecret = "$preMasterSecret".encryptAsymmetric(n, e);
      ...
      Future<void> dispose() async {
        ...
        try {
          await http.delete(removeUri, body: "$sessionKey".encryptAsymmetric(n, e));
  ...
  static Future<_HandshakeResult> _cachedHandshake(Uri uri) async {
    ...
    final handshakeResult = zoneExpando[baseUri] ??= await _newHandshake(baseUri);

    if (kDebugMode) {
      printBoxed(
        "Session Key: ${handshakeResult.$1}, \n"
            "Encryption Key: ${handshakeResult.$2}",
        "Cached Handshake result for ${baseUri.toString()}",
      );
    }

    if (handshakeResult case (final sessionKey, final encryptionKey, final dispose, final timer)) {
      timer.cancel();
      final newTimer = Timer(const Duration(minutes: 1), dispose.call);
  ...
  static Future<_HandshakeResult> _newHandshake(Uri baseUri) async {
    ...
    final timer = Timer(const Duration(minutes: 1), dispose.call);
