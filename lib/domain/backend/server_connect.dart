...
Future<(WebSocketChannel, MessageChannel, Stream<ServerEvent>)> connectToWebSocketServer(
  ...
  if (error != null) {
    if (isDebugMode) {
      printBoxed("Failed to connect to WebSocket server at $address: $error");
    }
  ...
  if (error2 != null) {
    if (isDebugMode) {
      printBoxed("Failed to establish WebSocket connection: $error2");
    }
    await dispose();
  ...
  if (error3 != null) {
    if (isDebugMode) {
      printBoxed("Failed to receive status from server: $error3");
    }
    await dispose();
...
Future<_WebSocketHandshakeResult> _webSocketHandshake(String landingAddress) async {
  ...
  if (error1 != null) {
    if (isDebugMode) {
      printBoxed("Failed to get WebSocket port: $error1");
    }
