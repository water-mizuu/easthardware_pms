...
class LoginPage extends StatelessWidget {
  ...
  List<SingleChildWidget> listeners(BuildContext context) {
    return [
          ...
          try {
            ...
            if (status == AuthenticationStatus.failure) {
              if (authState.loginAttempts >= 3) {
                ...
                context.read<AuthenticationBloc>().add(const AuthenticationResetEvent());
              }
      ...
      /// This listener handles authentication state changes whenever the login was successful.
      ///   It resets the login form state to its initial state.
      BlocListener<AuthenticationBloc, AuthenticationState>(
      ...
      BlocListener<LoginFormBloc, LoginFormState>(
        listenWhen: (p, c) => c.status == FormStatus.error && c.errorMessage != null,
        listener: (context, state) {
          showNotification(
            title: 'Login Error',
