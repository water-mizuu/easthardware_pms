...
- **Windows 11 Native Styling & PDF Reporting**: Built using the [`fluent_ui`](https://pub.dev/packages/fluent_ui) package for a premium, Windows 11-native design. Integrates interactive financial charts via [`fl_chart`](https://pub.dev/packages/fl_chart) and supports printing/saving professional invoices and purchase orders directly to PDF via the `pdf` and `printing` libraries.
