...
public class WindowManagerPlusPlugin: NSObject, FlutterPlugin {
    ...
    private func ensureInitialized(windowId: Int64) {
        if !_inited {
            ...
            // The issue is, by calling ensureInitialized, we are invalidating
    ...
    public func handleDefunct(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "ensureInitialized":
            if windowId >= 0 {
                ...
                result(
                    FlutterError(
                        code: "0", message: "Cannot skibidi! windowId >= 0 is required",
                        details: nil))
            }
            break
        default:
            print("Instance implemented: \(methodName)")
    ...
    public func staticHandle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        let methodName: String = call.method
        let args: [String: Any] = call.arguments as? [String: Any] ?? [:]
        print("Static method called: \(methodName) with args: \(args)")
        ...
        case "getAllWindowManagerIds":
            let keys = [Int64](
                WindowManagerPlus.windowManagers.keys.filter { key in
                    return WindowManagerPlus.windowManagers[key] != nil
                })
            result(keys)
            break
        default:
            print("Static not implemented: \(methodName)")
            result(FlutterMethodNotImplemented)
        }
    }
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "invokeMethodToWindow":
            if let targetWindowId = args["targetWindowId"] as? Int64,
                let wm = WindowManagerPlus.windowManagers[targetWindowId], let wm2 = wm
            {
                ...
                result(
                    FlutterError(
                        code: "0", message: "Cannot invokeMethodToWindow! targetWindowId not found",
                        details: nil))
            }
            break
